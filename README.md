...
flowchart TD
    ...
    C --> D["3단계: 현실주의 분석관 의사결정 체인 (gpt-4o-mini)<br>하소연 원문 + 실시간 식대 팩트(88,000원) + 4대 절대 철칙 결합 추론"]
    
    D --> E["4단계: Pydantic 구조화 파싱 (with_structured_output)<br>WeddingGiftVerdict 검증 객체 생성 (delivery_grade 필드 포함)"]
    
    E --> F["5단계: 최종 출력 및 Gradio UI 연동<br>- 성의도 등급: 3등급 (모바일 링크만 살포)<br>- 조사 식대: 88,000원<br>- 최종 행동: 계좌 송금 후 불참<br>- 추천 금액: 30,000원<br>- 현실 근거: 3등급 결례 지적 및 죄책감 해소<br>- 카톡 멘트: 정중한 선약 핑계 완성문"]
```
...
| **청첩장 성의도 등급** | **3등급 (모바일 링크만 살포)** | **1등급 (직접 식사 대접 및 종이 청첩장 전달)** | 성의도 3대 기준 자동 분류 |
