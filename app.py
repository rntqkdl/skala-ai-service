...
class WeddingGiftVerdict(BaseModel):
    ...
    recommended_amount: int = Field(
        description="최종 추천 축의금 금액 (원 단위 정수). 패스는 반드시 0, 불참송금은 30000 또는 50000, 직접참석 식사일 때만 식대 이상(10만 이상). 무성의 링크 전달은 5만 초과 절대 불가."
    )
...
[절대 정합성 규칙: 금액과 행동 지침 일치]
1. 성의도 등급별 축의금 상한 원칙 (1등급 >= 2등급 >= 3등급):
   - 1등급 (직접 식사 대접하며 청첩장 전달): 직접 참석 권장, 축의금 100,000원 - 200,000원
   - 2등급 (정성 통화/개인 메시지): 관계에 따라 직접 참석(100,000원) 또는 송금 불참(50,000원)
   - 3등급 (사전 연락 없이 단톡방/카톡 링크만 툭 전달): 상대방의 중대한 결례이므로 직접 참석 식사는 절대 금지합니다. '계좌 송금 후 불참(30,000원-50,000원)' 또는 '정중한 축하 인사만 전송(0원)'으로만 판정해야 합니다.
   - [필수 경고]: 3등급의 축의금이 50,000원을 초과하거나 1등급/2등급보다 커지는 역전 현상은 절대 불가합니다.
2. 행동 지침과 recommended_amount 값의 100% 일치:
   - attendance_decision이 '정중한 축하 인사만 전송(0원)'이면 recommended_amount는 반드시 0이어야 합니다.
   - attendance_decision이 '계좌 송금 후 불참'이면 recommended_amount는 반드시 30,000 또는 50,000이어야 합니다.
   - attendance_decision이 '직접 참석하여 식사'일 때만 recommended_amount가 식대 원가 이상(100,000원 이상)이 됩니다.
3. 행동 지침과 kakao_message_template 내용의 100% 일치 (모순 금지):
   - '직접 참석하여 식사' 판정 시: 절대 불참 핑계를 대지 말고, "기쁜 날 직접 찾아뵙고 진심으로 축하해 드리겠다"는 축하 및 참석 약속 멘트를 작성하세요.
   - '계좌 송금 후 불참' 또는 '패스' 판정 시: 상대방이 섭섭하지 않도록 품격 있는 선약/일정 핑계와 따뜻한 축하 문구를 작성하세요.
4. 식대 원가 하한선 원칙:
...
def process_wedding_inquiry(user_story: str):
    if not user_story.strip():
        return "상황을 입력해 주세요.", "0 원", "대기 중", "내용을 작성해 주시면 분석관이 판정표를 작성합니다.", ""
    ...
    return cost_text, amount_text, result.attendance_decision, result.reasoning_analysis, result.kakao_message_template

# 6. Gradio 인터랙티브 웹 UI (단일 텍스트 입력창 구조)
custom_css = """
...
with gr.Blocks(title="결혼식 축의금 손익 판독기") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            gr.Markdown("### ✍️ 상황 및 고민 입력")
            user_input = gr.Textbox(
                label="청첩장 수신 상황 (자유 서술)",
                placeholder="예시: 4년 동안 연락 없던 고교 동창이 단톡방에 청첩장 링크만 띡 보냈어. 예식장은 강남 아펠가모 선릉이라는데, 나 취준생이거든? 5만원 내고 밥 먹으러 가도 될까?",
                lines=5
            )
            with gr.Row():
                btn_ex1 = gr.Button("📋 예시 1: 손절각 동창 (링크 살포)", size="sm")
                btn_ex2 = gr.Button("📋 예시 2: 직속 사수 (호텔 식사 대접)", size="sm")
    ...
    submit_btn.click(
        ...
        inputs=user_input,
        outputs=[out_meal_cost, out_amount, out_decision, out_reasoning, out_kakao]
    )
