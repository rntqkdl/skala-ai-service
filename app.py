...
class WeddingGiftVerdict(BaseModel):
    ...
    reasoning_analysis: str = Field(
        description="설명 가능한 판정 이유: 1) [인간관계 밀도 분석 영수증(최근성35, 전우애30, 부채감25, 연차10)], 2) [하객 1인 대차대조표(식대 vs 축의금 = 신랑측 넷 손익)], 3) [심리적 면죄부 및 또래 2030 합의 벤치마크]를 명확한 소제목과 수치로 작성"
    )
...
7. 설명 가능한 판정 근거 작성 규칙 (reasoning_analysis, 사용자 100% 납득 원칙):
   - 판정 이유는 단순 줄글을 배제하고, 다음 3대 요소를 반드시 명확한 구조로 작성하여 사용자를 납득시키세요:
     1. [인간관계 밀도 분석 영수증 (총점 OO점 / 100점)]:
        - 최근 1년 교류 빈도 (35점 만점): OO점
        - 고락 극복 전우애 (30점 만점): OO점
        - 사전 식사/호의 수혜 (25점 만점): OO점
        - 누적 알고 지낸 기간 (10점 만점): OO점
     2. [하객 1인 기준 신랑측 손익 대차대조표]:
        - 예식장 1인 식대 원가와 추천 축의금을 대비하여 신랑측 넷(Net) 손익 계산 명시. (식사 참석 시 흑자/적자 여부, 불참 송금 시 순수 흑자 여부)
     3. [심리적 면죄부 및 2030 또래 벤치마크]:
        - 행동의 사회적 정당성을 보증하고, "동일 상황 2030 하객의 OO%가 동일한 결정 선택" 통계를 제시하여 죄책감을 완전히 해소할 것.
"""
